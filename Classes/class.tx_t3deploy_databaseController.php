...
class tx_t3deploy_databaseController
{
    ...
    public function updateStructureAction(array $arguments)
    {
        ...
        if ($isExecuteEnabled) {
            $result .= ($result ? PHP_EOL : '') . $this->executeUpdateStructureUntilNoMoreChanges($arguments,
                    $isRemovalEnabled);
        }
    ...
    protected function checkChangesSyntax($changes)
    {
        ...
        if ($checked != trim(strtoupper($checked))) {
            throw new Exception('Changes for schema_up seem to contain weird data, it starts with this:' . PHP_EOL . substr($changes,
                    0,
                    200) . PHP_EOL . '==================================' . PHP_EOL . 'If the file is ok, please add your conditions to file res/extensions/t3deploy/classes/class.tx_t3deploy_databaseController.php in t3deploy.');
        }
    }
